...
export default function AdminRewards() {
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto">
      <h1 className="text-[48px] font-serif text-gray-800 mb-8 border-b pb-4">Loyalty Rewards Management</h1>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        
        {/* LEFT COMPONENT */}
        <div>
          <h2 className="text-[36px] font-bold mb-6 flex items-center gap-2">
            <span className="bg-[#c3a2ab] text-white w-8 h-8 rounded-full flex items-center justify-center text-[22px]">1</span>
          ...
          <form onSubmit={addReward} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 space-y-5">
            <div>
              <label className="block text-[22px] font-medium text-gray-700 mb-1">Reward Name</label>
            ...
            <div className="flex gap-4">
              <div className="w-1/2">
                <label className="block text-[22px] font-medium text-gray-700 mb-1">Points Required</label>
              ...
              <div className="w-1/2">
                <label className="block text-[22px] font-medium text-gray-700 mb-1">Stock</label>
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-grow">
                <label className="block text-[22px] font-medium text-gray-700 mb-1">Emoji / Image Link</label>
              ...
              <div className="w-16 h-16 bg-gray-50 border-2 border-gray-100 rounded-lg flex items-center justify-center text-[44px] overflow-hidden shrink-0 mb-[2px]">
            ...
            {editingId && (
              <button type="button" onClick={() => { setEditingId(null); setFormData({ name: "", points_required: "", stock: "", image: "🎁" }); }} className="text-gray-500 hover:text-gray-700 w-full text-[22px] font-medium mt-2">
                Cancel Edit
              </button>
            )}
          </form>

          <h2 className="text-[36px] font-bold mt-12 mb-6 flex items-center gap-2">
            <span className="bg-green-500 text-white w-8 h-8 rounded-full flex items-center justify-center text-[22px]">2</span>
            Active Catalog
          </h2>
          {rewards.length === 0 ? (
              ...
              {rewards.map(r => (
                 <li key={r.id} className="border border-gray-100 p-4 rounded-xl bg-white shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
                   <div className="flex items-center gap-4">
                     <div className="w-16 h-16 bg-gray-50 rounded-lg flex items-center justify-center text-[48px] overflow-hidden shrink-0">
                     ...
                     <div>
                       ...
                       <p className="text-[22px] text-[#c3a2ab] font-bold">{r.points_required} Pts</p>
                     </div>
                   </div>
                   <div className="flex flex-col items-end gap-2">
                     <div className="text-right">
                       <span className={`inline-block px-3 py-1 rounded-full text-[20px] font-bold ${r.stock > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                         Stock: {r.stock}
                       </span>
                       <p className="text-[20px] text-gray-400 mt-1">ID: {r.id.slice(-4)}</p>
                     </div>
                     <div className="flex gap-2">
                       <button onClick={() => handleEdit(r)} className="p-2 hover:bg-blue-50 text-blue-600 rounded-lg transition-colors" title="Edit">
                         <span className="material-symbols-outlined notranslate text-[30px]">edit</span>
                       </button>
                       <button onClick={() => handleDelete(r.id)} className="p-2 hover:bg-red-50 text-red-600 rounded-lg transition-colors" title="Delete">
                         <span className="material-symbols-outlined notranslate text-[30px]">delete</span>
                       </button>
                     </div>
                   </div>
                 </li>
        ...
        <div>
          <div className="flex justify-between items-center mb-6 border-b pb-2 pt-2">
            <h2 className="text-[36px] font-bold flex items-center gap-2">
              <span className="bg-amber-500 text-white w-8 h-8 rounded-full flex items-center justify-center text-[22px]">3</span>
              Recent Redemptions
            </h2>
            <div className="flex gap-2">
              <button 
                onClick={() => setFilter("PENDING")} 
                className={`px-3 py-1.5 rounded-full text-[20px] font-bold transition-colors ${filter === "PENDING" ? "bg-amber-100 text-amber-800" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
              >
                To Ship ({redemptions.filter(r => r.status === "PENDING").length})
              </button>
              <button 
                onClick={() => setFilter("SHIPPED")} 
                className={`px-3 py-1.5 rounded-full text-[20px] font-bold transition-colors ${filter === "SHIPPED" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
              >
            ...
            {filteredRedemptions.map(r => (
              <div key={r.id} className="bg-white border hover:border-[#c3a2ab] p-6 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.03)] transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <p className="font-bold text-[26px] text-gray-800 mb-1">{r.reward.name}</p>
                    <p className="text-[22px] text-gray-600">Claimed by: <strong className="text-black">{r.user.name || "Unknown"}</strong> <span className="text-[20px] text-gray-400">({r.user.email})</span></p>
                    {/* Shipping Address */}
                    <div className="mt-3 text-[20px] text-gray-600 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    {r.tracking_number && (
                      <div className="mt-2 text-[20px] font-medium text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg inline-flex items-center gap-1 border border-blue-100">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`inline-block px-3 py-1.5 rounded-full text-[20px] font-bold tracking-wider ${r.status === "PENDING" ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}`}>
                      {r.status}
                    </span>
                ...
                <div className="flex items-center justify-between text-[20px] text-gray-500 border-t pt-4 mt-2">
                  ...
                  {r.status === "PENDING" && (
                    ...
                    >
                      <span className="material-symbols-outlined notranslate text-[22px]">local_shipping</span>
                      Mark as Shipped
                    </button>
                  )}
                </div>
